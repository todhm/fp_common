...
def fetch_realestate_data(
    api_key: str, start_number: int, end_number: int
) -> List[RealEstate]:
    ...
    for row in data_list:
        ...
        new_row["OBJ_AMT"] = float(new_row["OBJ_AMT"]) * 10000
        new_row["BLDG_AREA"] = float(new_row["BLDG_AREA"])
        new_row["TOT_AREA"] = float(new_row["TOT_AREA"])
        new_row["DEAL_YMD_DT"] = dt.strptime(new_row["DEAL_YMD"], "%Y%m%d")
        new_row["FLOOR"] = int(new_row["FLOOR"]) if new_row.get("FLOOR") else None
        new_row["BUILD_YEAR"] = (
            int(new_row["BUILD_YEAR"])
            if new_row.get("BUILD_YEAR") and new_row.get("BUILD_YEAR") != "null"
            else None
        )
